...
def evaluate_forecasts(actual, predicted):
    ...
    print('actual.shape[0]:{}, actual.shape[1]:{}'.format(
...
def sliding_window(train, sw_width=7, n_out=7, in_start=0):
    ...
    '''
    data = train.reshape(
        (train.shape[0] * train.shape[1], train.shape[2]))  # 将以周为单位的样本展平为以天为单位的序列
    X, y = [], []
...
def main_run(dataset, sw_width, days, name, in_start, verbose, epochs, batch_size):
    ...
    # 训练模型
    model = cnn_model(train, sw_width, in_start, verbose_set=0,
                      epochs_num=20, batch_size_set=4)
    # 计算RMSE
...
if __name__ == "__main__":
    dataset = pd.read_csv('./input/test/household_power_consumption_days.csv', header=0,
